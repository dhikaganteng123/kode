...
function hitung_cf(PG, G) {
  let cf_total = 0;
  let cfA =
    PG["PG1"] * G["G1"] + PG["PG2"] * G["G2"] * (1 - PG["PG1"] * G["G1"]);
  let cfB = PG["PG2"] * G["G2"] + cfA * (1 - PG["PG2"] * G["G2"]);
  let cfC = PG["PG3"] * G["G3"] + cfB * (1 - PG["PG3"] * G["G3"]);
  let cfD = PG["PG4"] * G["G4"] + cfC * (1 - PG["PG4"] * G["G4"]);
  let cfE = PG["PG5"] * G["G5"] + cfD * (1 - PG["PG5"] * G["G5"]);
...
app.post("/result", (req, res) => {
  ...
  for (let key in req.body.symptoms) {
    let symptom = req.body.symptoms[key];
    let selectedValue = symptom.value;
    ...
    if (
      ...
      PilihanUser[gejalaKey] = {
        nama: Pilihan[gejalaKey][pilihanKey]["Pilihan"],
        "Nilai Pakar": Pilihan[gejalaKey][pilihanKey]["Nilai"],
      };
    } else {
